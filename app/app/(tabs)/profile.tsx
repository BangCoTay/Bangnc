...
export default function ProfileScreen() {
  ...
  const { myCharacters, favorites, fetchMyCharacters, fetchFavorites } = useCharacterStore();
  ...
  const MenuItem = ({ icon, label, onPress, danger }: { icon: string; label: string; onPress: () => void; danger?: boolean }) => (
    <TouchableOpacity style={styles.menuItem} onPress={onPress} activeOpacity={0.7}>
      <Ionicons name={icon as any} size={22} color={danger ? colors.error : colors.textSecondary} />
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      >
        ...
        <View style={styles.statsRow}>
          <StatCard icon="create-outline" value={myCharacters.length} label="Created" />
          <StatCard icon="heart-outline" value={favorites.length} label="Favorites" />
          <StatCard icon="wallet-outline" value={user?.coin_balance || 0} label="Coins" />
        </View>
          ...
          <TouchableOpacity onPress={() => router.push('/subscription')} activeOpacity={0.85}>
            ...
            >
              ...
              <View style={{ flex: 1, marginLeft: spacing.md }}>
                ...
                <Text style={styles.upgradeBannerSubtitle}>Unlimited messages & memory</Text>
              </View>
        ...
        <View style={styles.menuSection}>
          ...
          <MenuItem icon="create-outline" label="My Characters" onPress={() => {}} />
          <MenuItem icon="heart-outline" label="Favorites" onPress={() => {}} />
          <MenuItem icon="card-outline" label="Subscription" onPress={() => router.push('/subscription')} />
...
const styles = StyleSheet.create({
  ...
  profileHeader: { paddingHorizontal: spacing.base, paddingTop: spacing.lg, marginBottom: spacing.xl },
  avatarSection: { flexDirection: 'row', alignItems: 'center' },
  avatar: {
    width: 72, height: 72, borderRadius: 36,
    justifyContent: 'center', alignItems: 'center',
  },
  avatarText: { fontSize: typography.size['2xl'], fontWeight: '700', color: '#fff' },
  userInfo: { marginLeft: spacing.base, flex: 1 },
  displayName: { fontSize: typography.size.xl, fontWeight: '700', color: colors.textPrimary },
  username: { fontSize: typography.size.sm, color: colors.textSecondary, marginTop: 2 },
  tierBadge: {
    backgroundColor: colors.primaryMuted, paddingHorizontal: spacing.sm,
    paddingVertical: 2, borderRadius: borderRadius.sm, alignSelf: 'flex-start', marginTop: spacing.xs,
  },
  tierText: { fontSize: typography.size.xs, color: colors.primaryLight, fontWeight: '600' },
  bio: { fontSize: typography.size.sm, color: colors.textSecondary, marginTop: spacing.md, lineHeight: typography.lineHeight.sm },
  ...
  statValue: { fontSize: typography.size.xl, fontWeight: '700', color: colors.textPrimary, marginTop: spacing.xs },
  ...
  upgradeBannerTitle: { fontSize: typography.size.base, fontWeight: '600', color: '#fff' },
  upgradeBannerSubtitle: { fontSize: typography.size.xs, color: 'rgba(255,255,255,0.7)' },
  ...
  menuDivider: { height: 1, backgroundColor: colors.divider, marginHorizontal: spacing.base },
});
